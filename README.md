...
| **超宽屏适配**   | 在 21:9 / 32:9 下内容区域保持居中并限制最大宽度，额外横向空间用于展示动态背景与留白，不会无限拉伸内容   |
...
- **当前示例视频**: 书籍广告、真人模拟（支持上下键切换）
- **注释文字**: 每个视频附带说明文案

#### 超宽屏显示说明（21:9 / 32:9）

- 页面外层会铺满整个视口，但主要内容区保持居中布局
- 默认 `.story-page` 宽度限制为 `min(1180px, 95vw)`，在 `min-width: 1800px` 时放宽为 `min(1680px, 94vw)`
- Page 5 视频容器默认宽度为 `min(1080px, 94vw)`，超宽屏时提升为 `min(1560px, 92vw)`，并始终保持 `16:9`
- 横向多余空间主要显示全局动态背景与留白，不会导致内容被无限拉伸
